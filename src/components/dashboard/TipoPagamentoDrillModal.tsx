...
export interface TipoDrillSelection {
  tipo: string;        // e.g. "FORNECEDOR"
  tipoLabel: string;   // e.g. "Fornecedor"
...
interface Props {
  selection: TipoDrillSelection | null;
  onClose: () => void;
}

const TipoPagamentoDrillModal = ({ selection, onClose }: Props) => {
  ...
  const filtered = useMemo(() => {
    ...
    let records = (drillData as DrillRecord[]).filter((r) => {
      ...
      const matchTipo = r.tipo === selection.tipo;
      const matchCompany = selection.company === "all" || r.empresa === selection.company;
      return matchPeriod && matchTipo && matchCompany;
    });
  ...
  const companyLabel = selection.company === "all" ? "Total" : selection.company;
      ...
      <DialogContent className="max-w-4xl max-h-[85vh] flex flex-col p-0 gap-0">
        <DialogHeader className="p-6 pb-4 border-b border-border space-y-3">
          ...
          <DialogTitle className="text-lg font-bold">
            {selection.tipoLabel.toUpperCase()} — {companyLabel} — {periodLabel(selection.period)}
          </DialogTitle>
...
export default TipoPagamentoDrillModal;
